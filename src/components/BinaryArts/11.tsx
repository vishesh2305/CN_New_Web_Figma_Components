...
const Elevenascii: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      canvasRef.current.textContent = text;
    };
      ...
      for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
          if (
            ...
            const erosion = (time * 0.0067) % blockSize; // looping erosion
            grid[y][x] =
              innerDist > erosion ? "1" : Math.random() > 0.8 ? "1" : "0";
          } else {
            // Flow outside
            const dx = x - width / 2;
            const dy = y - height / 2;
